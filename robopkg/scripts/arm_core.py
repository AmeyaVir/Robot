...
from robopkg.msg import Angles
...
def readCurrentAngles():
	ra = {}
	ra['s1'] = arm.get_servo_angle(0)
	ra['s2'] = arm.get_servo_angle(1)
	ra['s3'] = arm.get_servo_angle(2)
	ra['s4'] = arm.get_servo_angle(3)
...
def writeSingleAngle(num, angle):
	...
	s3_max = 170 - arm.get_servo_angle(1)
	if num == 1:
		if angle > 160:
			angle = 160
		elif angle < 20:
	...
	arm.set_servo_angle(num-1, angle)
...
def writeAnglesCallback(servos):
	...
	arm.set_servo_angle(0, servo['s1'])
	arm.set_servo_angle(1, servo['s2'])
	arm.set_servo_angle(2, servo['s3'])
	arm.set_servo_angle(3, servo['s4'])

	print 'Movement: Moved Once'


# attach or detach arm function once received data from topic
def attchCallback(attachStatus):
	data_input = attachStatus.data

	if data_input.lower() == 'attach' :
		arm.attach_all_servos()
		print 'Arm: Attach'
	elif data_input.lower() == 'detach':
		arm.detach_all_servos()
		print 'Arm: Detach'
	else:
		pass
...
def moveToTimeAndS4Callback(coordsAndTS4):
	...
	if s4 > 180: s4 = 180
	if s4 <0 : s4 =0
	arm.set_position(x, y, z, s4, 0, time, 0, 0)
	print 'Movement: Moved Once'
	pass
...
def listener():
	...
	rospy.Subscriber("arm_status",String, attchCallback)
...
def processFailedNum(failed_number):
	...
	if failed_number == 20:
		print 'Arm: Please Connect Arm first '
